...
# Introduction
In this project, we decided to study a particular relationship between personal variable and US citizen's income... 

* TODO: please, complete the introduction... *
...
# TODO: @Connor, please explain how you did the transformations
par(mfrow=c(1,2))
plot(df$IND)
plot(df$IND_revised)
```
...
```
We transformed industry variable by putting them into appropriate industry category
...
```
Transformed PWSTATE2 by categorizing them into four regions of united states.
...
```
Transformed HCOVANY variable by assigning descripitve names rather than just simple numbers.
...
After weighting the data, we still left with around 10,000 rows. 
```{r}
...
distributions(dt)
```
We sample the dataset and make sure to maintain the distribution properties of the variables.
```{r}
set.seed(1337)
...
summary(mlr_bs)
```
Adjusted $\bar{R}^2$ turned out to be $0.3108$, which is slightly better than the first model. 
...
value_bs
```
Running Box-Cox test reveals that we should use $\lambda = 0.01414141$ for our transformation on our dependent variable which is income. As you can see from above plot, maximum likelihood estimation has the highest peak at $\lambda = 0.01414141$.
...
The transormed model has $\bar{R}^2$ equal to $0.6029$, which is a big improvement from all previous models. 
...
```
Histogram plot above can be said appproximately normal, further indicating that the normality assumption holds for the transformed model. However, we might need to run AD test to further determine whether normality condition is met. 
...
n <- nrow(orig_data)
f_50 <- qf(0.50, df1 = k+1, df2 = n - k - 1)
f_80 <- qf(0.20, df1 = k+1, df2 = n - k - 1)
any(cook > f_50, na.rm = T)
```
There seems to be no influential observations according to cook's distance test described in the textbook. None of the observations have cook's distance values greater than $F_{[0.5]}$, but every observations have cook's distance values less than $F_{[0.8]}$. Some sources suggest different cut-off values to use for spotting highly influential points. 
...
(a) Assume the transformation to be a natural logarithm transformation (since the value of $\lambda = 0.01414141$ in our transformation was close to zero);
