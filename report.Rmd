...
<<<<<<< HEAD
Our preliminary model has adjusted R^2 = 0.3092 and F-test indicates that the overall model is significant at alpha=0.05. However, plotting residuls reveal that preliminary model doesn't satisfy the regression assumptions. Manual model doesn't do any better than preliminary model in terms of adjusted R^2 because they have same adjusted R^2 value. Running vif test reveal that preliminary model doesn't seem to exhibit any serious multicollinearity among dependent variables. We can further narrow down our model by runnning both backward elimination and stepwise regression on preliminary model. Both methods leave us with SEX, AGE, NUM_HOURS_WEEK, EDUC, and DIVISION variables to run regression on income variable. Automatic model does slightly better than preliminary model in terms of adjusted R^2 because now adjusted R^2 is 0.3108, but plot of residuals shows that automatic model still doesn't satisfy the regression assumption. In order to meet the regression assumptions requirement, we had to transform our income, depedent variable, using box-cox transformation mathod. Applying box-cox transformation on automatic model gives us transformed automatic model, which has residuals normally distributed and adjusted R^2 equal to 0.503. However, plot of residuals doesn't seem to quite satisfy the regression assumption even though distribution of residuals way more closely resembles noraml distribution than preliminary or automatic model. 

It seems that we can do better by applying boxcox transformation on preliminary model(the original model) first before applying stepwise regression or backward elimination method. If we box-cox transform our income variable first, we get transformed preliminary model, which has adjusted R^2 is equal to 0.5836. Then, we sequentially apply stepwise regression or backward elimination method on transformed preliminary model. Both methods give us two identical models using same independent variables. Finally, we can test for influential observations wrt both y and x values. Cook's distance measure test indicate that we have some influential observations wrt both x and y values. Likewise, applying studentized residuals statistical test on automatic transformed preliminary model reveals that we have 28 potential outliers that need to be taken care of. We decided to exclude those 28 outliers and ran a new regression using reduced data set. At last, we have achieved our best model so far. automatic transformed preliminary model with 28 outliers omitted has adjusted R^2 equal to 0.6783, and plots of residuals approximately satisfies the regression assumption even though we have few observations that are possibly outliers wrt to either y or x values.  
=======
...
>>>>>>> a24d91a8fd2313066d23de5af3fffd6227e6ff31
